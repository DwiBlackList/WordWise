import { j as jsplumbExports } from "./jsplumb-CVW381dp.js";
class NodeManager {
  ...
  initializeJsPlumb(containerId) {
    jsplumbExports.jsPlumb.setContainer(containerId);
    jsplumbExports.jsPlumb.importDefaults({
  ...
  makeNodeDraggable(element) {
    jsplumbExports.jsPlumb.draggable(element, {
      containment: "parent",
      grid: [this.gridSize, this.gridSize],
      drag: () => {
        jsplumbExports.jsPlumb.repaintEverything();
      }
    });
  }
  ...
  addEndpoints(node, type2) {
    if (type2 !== "start") {
      jsplumbExports.jsPlumb.addEndpoint(node, {
        anchor: "Left",
        isTarget: true,
  ...
  removeNode(nodeId) {
    ...
    jsplumbExports.jsPlumb.removeAllEndpoints(node.element);
  }
  serialize() {
